...
<div class="site-home">
    <section class="site-hero">
        <div class="site-hero__copy">
            <p class="site-pill">para {{ $coupleName }}</p>
            <h1 class="site-hero__title">{{ $heroTitle }}</h1>
            <p class="site-hero__lead">{{ $heroSubtitle }}</p>
            <p class="site-hero__kicker">{{ $nicknameLine }}</p>
            ...
            <div class="site-hero__panel">
                ...
                <div class="site-panel" x-data="loveTimer('{{ $startedAt }}')" x-init="init">
        ...
        <div class="site-timeline-grid">
            @foreach ($timelineEvents as $event)
                <article class="site-card">
                    <p class="site-card__meta">{{ $event->happened_at?->format('d/m/Y') ?? 'memória' }}</p>
                    <h3 class="site-card__title">{{ $event->icon }} {{ $event->title }}</h3>
                    <p class="site-card__text">{{ $event->description }}</p>
                </article>
            @endforeach
        </div>
    </section>
    ...
    <section id="playlist" class="site-section">
        ...
        <div class="site-player" x-data="playlistPlayer(@js($songs->map(fn($song) => ['title' => $song->title, 'artist' => $song->artist, 'url' => Storage::url($song->audio_path), 'cover' => $song->cover_path ? Storage::url($song->cover_path) : null])->values()))" x-init="init">
                ...
                <div class="site-track-list">
                    @foreach ($songs as $song)
                        <div class="site-track">
                            ...
                            <div class="site-panel__title" style="margin-top: 0.55rem; font-size: 1.1rem;">{{ $song->title }}</div>
                            <div class="site-track__artist" style="margin-top: 0.3rem;">{{ $song->artist }}</div>
                        </div>
                    @endforeach
                </div>
            </div>
    ...
    <section class="site-section">
        ...
        <div class="site-grid">
            @forelse ($reasons as $reason)
                <x-emotional-card :title="$reason->title" :text="$reason->subtitle ? $reason->subtitle . ' — ' . $reason->body : $reason->body" />
            @empty
    ...
    <section id="carta" class="site-section">
        ...
        <div class="site-letter">
            <div class="site-letter__body">{!! nl2br(e($letterBody)) !!}</div>
        </div>
    </section>
    ...
    <section class="site-section">
        ...
        <div class="site-contract">
            <div class="site-control-row" style="margin-top: 0;">
                @if ($contractPdfPath)
                    <a class="site-button" href="{{ Storage::url($contractPdfPath) }}" download>baixar PDF</a>
                @else
    ...
    <section class="site-section">
        ...
        <div class="site-secret-grid">
            @foreach ($secretCards as $card)
                <x-secret-card :card="$card" />
            @endforeach
        </div>
    </section>
