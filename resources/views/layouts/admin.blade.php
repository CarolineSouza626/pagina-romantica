...
<head>
    ...
    <title>Admin • {{ $title ?? 'Painel' }}</title>
        ...
        <aside class="glass-panel h-fit overflow-hidden p-0 lg:sticky lg:top-6">
            ...
            <nav class="space-y-2 p-4 text-sm">
                @foreach ($navigation as $item)
                    <a
                        class="flex items-center gap-3 rounded-2xl px-4 py-3 transition duration-300 hover:-translate-y-0.5 hover:bg-white/90 {{ request()->routeIs($item['route']) ? 'bg-white text-rosewood shadow-sm ring-1 ring-white' : 'text-ink/80' }}"
                        href="{{ route($item['route']) }}"
                    >
                        <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-paper-100 text-base text-rosewood">{{ $item['icon'] }}</span>
