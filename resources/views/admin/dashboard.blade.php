            ...
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                @foreach ($stats as $label => $value)
                    <div class="paper-card p-5">
                        <p class="text-xs uppercase tracking-[0.3em] text-rosewood/60">{{ $label }}</p>
                        <div class="mt-4 text-4xl font-serif tracking-tight">{{ $value }}</div>
                    </div>
                @endforeach
            </div>
            ...
            <div class="paper-card mt-6 p-6">
                ...
                <div class="mt-5 grid gap-4 md:grid-cols-2">
                    <div class="rounded-[1.5rem] bg-paper-50 p-5">
                        ...
                        <p class="mt-2 text-lg font-medium">{{ $settings->get('couple_name')?->value ?? 'não definido' }}</p>
                    </div>
                    <div class="rounded-[1.5rem] bg-paper-50 p-5">
                        <p class="text-sm text-ink/60">Linha de apelidos</p>
                        <p class="mt-2 text-lg font-medium">{{ $settings->get('nickname_line')?->value ?? 'não definido' }}</p>
                    </div>
                </div>
            </div>
        </div>
